...
function CSVImport() {
  ...
  const transformData = () => {
    return csvData.map((row, index) => {
      ...
      const importResult = { row: index + 1, status: 'success', error: null, ...row };
      ...
      return { ...transformedRow, importResult };
    });
  };
  ...
  const handleSaveMappings = async () => {
    if (validateData()) {
      ...
      try {
        ...
        const activitiesDataString = JSON.stringify(transformedData); // Stringify the data
        console.log('activities_data:', activitiesDataString); // Log the data

        const { data, error } = await supabase
          .rpc('import_activities_from_csv', { activities_data: activitiesDataString });
